...
1. Настроить роутинг по локалям (например, `/en`, `/es`, `/pt-BR`)
2. Добавить компоненты для переключения языка
3. Реализовать автоматическое определение локали
4. Сгенерировать необходимые конфигурационные файлы
...
2. Отправляет его провайдеру LLM для перевода
3. Записывает переведённый контент обратно в файловую систему
4. Создаёт файл `i18n.lock` для отслеживания завершённых переводов (предотвращает избыточную обработку)
...
Поле `provider` необязательно (по умолчанию — Lingo.dev Engine). Для индивидуальных LLM-провайдеров:
...
**Разработка:** `npm run dev` (используется псевдопереводчик, без вызова API)
...
- Нулевая нагрузка на производительность рантайма
- Нет ключей переводов и JSON-файлов
- Нет функций `t()` или компонентов-обёрток `<T>`
- Автоматическое определение переводимых строк в JSX
...
- `pseudotranslator`: режим разработки с тестовыми переводами (без затрат на API)
- `real`: генерация реальных переводов с использованием LLM
- `cache-only`: продакшн-режим с использованием заранее сгенерированных переводов из CI (без запросов к API)
...
1. **Issues:** [Сообщить о баге или предложить фичу](https://github.com/lingodotdev/lingo.dev/issues)
2. **Pull Requests:** [Отправить изменения](https://github.com/lingodotdev/lingo.dev/pulls)
   ...
   - До отправки убедитесь, что все тесты проходят
...
**Формат BCP-47 для локалей:** `language[-Script][-REGION]`

- `language`: ISO 639-1/2/3 (маленькие): `en`, `zh`, `bho`
- `Script`: ISO 15924 (с заглавной): `Hans`, `Hant`, `Latn`
