...
**Lingo.dev Compiler** - це безкоштовне проміжне програмне забезпечення з відкритим кодом, розроблене для того, щоб зробити будь-який React додаток багатомовним під час збірки без необхідності внесення змін до існуючих React компонентів.
...
### Що всередині цього репозиторію?
...
| **CI/CD**    | Автоматичне збереження перекладів при кожному пуші + створення pull request за потреби  | [/ci](https://lingo.dev/ci)             |
...
[Перейдіть до документації →](https://lingo.dev/cli), щоб дізнатися, як налаштувати.

---

### 🔄 Lingo.dev CI/CD

Автоматично випускайте ідеальні переклади.
...
Підтримує ваш репозиторій в актуальному стані, а ваш продукт багатомовним без ручних кроків.
...
Миттєвий переклад динамічного контенту для кожного запиту.
...
Якщо вам подобається те, що ми робимо, поставте нам ⭐ і допоможіть нам досягти 5000 зірок! 🌟
